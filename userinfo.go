...
func createUserInfo(c *gin.Context, db *sql.DB) {
	var user UserInfo
	...
	query := "INSERT INTO userinfo (user_auth, donate_tier, expire_time) VALUES (?, ?, ?)"
	_, err := db.Exec(query, user.UserAuth, user.DonateTier, user.ExpireTime)
	...
	c.JSON(http.StatusCreated, gin.H{"message": fmt.Sprintf("User %s has been created", user.UserAuth)})
}
